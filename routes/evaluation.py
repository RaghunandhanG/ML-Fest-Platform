...
All endpoints live under /evaluate/ and automatically judge submissions.
"""
...
def _success(flag: str, detail: str = "") -> dict:
    return {"result": "success", "flag": flag, "detail": detail}


def _failure(reason: str) -> dict:
    return {"result": "failure", "flag": None, "detail": reason}


# ── Challenge 1 — Data Poisoning ─────────────────────────────────────────────

@router.post("/challenge-1", summary="Challenge 1: Data Poisoning")
async def evaluate_challenge_1(file: UploadFile = File(..., description="tampered_data.csv")):
    ...
    confidence = {"citizen_confidence": round(float(proba[0]), 4), "thief_confidence": round(float(proba[1]), 4)}

    if prediction == 0:
        return JSONResponse({"result": "success", "flag": CHALLENGE_1_FLAG,
                             "message": "The Gatekeeper has been fooled! The thief is classified as a Citizen.", **confidence})
    return JSONResponse({"result": "failure", "flag": None,
                         "message": "The thief is still correctly identified. The Gatekeeper was not fooled.", **confidence})


# ── Challenge 2 — Constrained Data Poisoning ─────────────────────────────────

@router.post("/challenge-2", summary="Challenge 2: Constrained Data Poisoning")
async def evaluate_challenge_2(file: UploadFile = File(..., description="tampered_data.csv")):
    if not file.filename.endswith(".csv"):
        raise HTTPException(400, "Please upload a CSV file.")
    ...
    if len(submitted) != len(original):
        return JSONResponse({"result": "failure", "flag": None, "message": "Row count does not match the original dataset."})
    ...
    if not last_orig.equals(last_sub):
        return JSONResponse({
            "result": "failure", "flag": None, "tampering_detected": True,
            "tampered_location": "last row (ID = 10)",
            "message": "\u26a0\ufe0f Tampering detected! The last row (ID = 10) has been modified. Submission rejected.",
        })
    ...
    if not original["Label"].reset_index(drop=True).equals(submitted["Label"].reset_index(drop=True)):
        return JSONResponse({
            ...
            "message": "\u26a0\ufe0f Tampering detected! The Label column has been modified. Submission rejected.",
        })
    ...
    if prediction == 0:
        return JSONResponse({"result": "success", "flag": CHALLENGE_2_FLAG,
                             "message": "The Gatekeeper was fooled again! The thief slipped past undetected.", **confidence})
    return JSONResponse({"result": "failure", "flag": None,
                         "message": "The thief is still correctly identified. The Gatekeeper held its ground.", **confidence})


# ── Challenge 3 — Fix the Corrupted Model ────────────────────────────────────

@router.post("/challenge-3", summary="Challenge 3: Fix the Corrupted Model")
async def evaluate_challenge_3(file: UploadFile = File(..., description="fixed_model.pkl")):
    ...
    if accuracy == 1.0:
        return JSONResponse({"result": "success", "flag": CHALLENGE_3_FLAG,
                             "message": "The Gatekeeper is fully restored! Every single record is classified correctly.", "accuracy": accuracy})
    return JSONResponse({"result": "failure", "flag": None,
...
async def evaluate_challenge_4(file: UploadFile = File(..., description="predictions.csv")):
    if not file.filename.endswith(".csv"):
        raise HTTPException(400, "Please upload a CSV file.")
    ...
    if auc >= CHALLENGE_4_AUC_THRESHOLD:
        return JSONResponse({
            "result": "success", "flag": CHALLENGE_4_FLAG,
            "message": "The Sentinel is awake! Your model generalises well on the test set.",
            "roc_auc": auc, "accuracy": accuracy,
        })
    return JSONResponse({
        "result": "failure", "flag": None,
        "message": f"The Sentinel is still struggling. ROC-AUC: {auc:.4f} (need \u2265 {CHALLENGE_4_AUC_THRESHOLD}). Accuracy: {accuracy:.4f}. Keep debugging the pipeline.",
        "roc_auc": auc, "accuracy": accuracy,
    })
...
async def evaluate_challenge_5(file: UploadFile = File(..., description="model.pkl")):
    if not file.filename.endswith(".pkl"):
        raise HTTPException(400, "Please upload a .pkl model file.")
    ...
    if is_perfect:
        return JSONResponse({
            "result": "success", "flag": CHALLENGE_5_FLAG,
    ...
    return JSONResponse({
        "result": "failure", "flag": None,
        "message": f"Not a perfect fit. R\u00b2 = {r2} (need exactly 1.0). Max absolute error = {max_abs_error} (need exactly 0). MSE = {mse} (need exactly 0).",
        "r2_score": r2, "mse": mse, "max_abs_error": max_abs_error, **weights_info,
    })
